...
class APITestHelper:
    ...
    @staticmethod
    def mock_yfinance_ticker(ticker: str = 'TEST', 
                           mock_responses: Optional[Dict[str, Any]] = None) -> Mock:
        """
    ...
    @staticmethod
    def assert_api_data_consistency(data1: Dict[str, Any], data2: Dict[str, Any], 
        ...
        for key in common_keys:
            ...
            if isinstance(val1, (int, float)) and isinstance(val2, (int, float)):
                if abs(val1 - val2) > tolerance * max(abs(val1), abs(val2), 1):
                    inconsistencies.append(
                        f"Numeric value mismatch for '{key}': {val1} vs {val2}"
                    )
            ...
            # Handle None comparisons
                ...
                inconsistencies.append(
                    f"Value mismatch for '{key}': {val1} vs {val2}"
                )
