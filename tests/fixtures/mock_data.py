...
class MockDataGenerator:
    ...
    def generate_fcf_data(years: int = 5, include_negative: bool = True) -> Dict[str, List[float]]:
        ...
        for fcf_type in ['FCFF', 'FCFE', 'LFCF']:
            values = []
            current_fcf = base_fcf + np.random.normal(0, 20)  # Different starting point for each type
    ...
    def generate_financial_metrics(num_years: int = 3) -> Dict[str, List[float]]:
        ...
        metrics = {
            ...
            'current_liabilities': [200 * (1.03 ** i) + np.random.normal(0, 20) for i in range(num_years)],
            'total_assets': [1000 * (1.08 ** i) + np.random.normal(0, 80) for i in range(num_years)],
            
            # Cash flow metrics
            'cash_from_operations': [250 * (1.07 ** i) + np.random.normal(0, 25) for i in range(num_years)],
            ...
            'cash_from_financing': [-20 * (1.15 ** i) + np.random.normal(0, 15) for i in range(num_years)],
        }
    ...
    def generate_sensitivity_analysis_data() -> Dict[str, Any]:
        ...
        return {
            ...
            'current_price': 150.0,
            'upside_downside': [
                [(val / 150.0 - 1) for val in row] for row in valuations
            ]
        }
