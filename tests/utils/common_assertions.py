...
def assert_numerical_values_valid(values: List[Union[float, int]], 
...
def assert_market_data_fields(market_data: Dict[str, Any], 
    ...
    for field in required_fields:
        ...
        if field in ['currentPrice', 'marketCap', 'sharesOutstanding', 
                    'enterpriseValue', 'totalDebt', 'totalCash']:
...
def assert_date_sequence_valid(dates: List[Any]) -> None:
    ...
    for i in range(1, len(comparable_dates)):
        assert comparable_dates[i] >= comparable_dates[i-1], \
            f"Dates are not in chronological order: {comparable_dates[i-1]} > {comparable_dates[i]}"


def assert_percentage_in_range(percentage: float, min_val: float = -1.0, max_val: float = 10.0) -> None:
    """
    ...
    assert min_val <= percentage <= max_val, \
...
def assert_dcf_results_valid(dcf_results: Dict[str, Any]) -> None:
    ...
    for field in expected_fields:
        if field in dcf_results:
            ...
            if value is not None:
                assert isinstance(value, (int, float)), \
                    f"DCF field '{field}' must be numeric"
...
def assert_no_duplicate_processing(results1: Any, results2: Any, tolerance: float = 1e-10) -> None:
    ...
    if isinstance(results1, dict) and isinstance(results2, dict):
        ...
        if isinstance(results1, float) or isinstance(results2, float):
            assert abs(results1 - results2) <= tolerance, \
                f"Floating point values differ by more than tolerance: {results1} vs {results2}"
        else:
            assert results1 == results2, f"Integer values must be exactly equal: {results1} vs {results2}"
