...
class E2EApiTester:
    ...
    def test_unified_adapter_e2e(self):
        ...
        try:
            ...
            request = FinancialDataRequest(
                ticker="AAPL", data_types=['price', 'fundamentals'], force_refresh=True
            )
        ...
        try:
            ...
            if response.success and response.data:
                fcf_calculated = any(
                    ...
                    for key in ['free_cash_flow', 'fcf_calculated', 'operating_cash_flow']
                )
    ...
    def _load_api_config(self, source_name: str) -> Optional[DataSourceConfig]:
        ...
        try:
            config_file = os.path.join(self.base_path, "config/data_sources_config.json")
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        avg_duration = (
            sum(r.duration for r in self.test_results) / total_tests if total_tests > 0 else 0
        )
        ...
        for api_name, results in api_results.items():
            ...
            report["api_breakdown"][api_name] = {
                ...
                "avg_duration": (
                    sum(r.duration for r in results) / api_total if api_total > 0 else 0
                ),
        ...
        if fcf_success_count > 0:
            report["recommendations"].append(
                f"✅ FCF calculation working on {fcf_success_count} test(s)"
            )
        else:
