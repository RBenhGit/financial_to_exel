...
def example_basic_usage():
    ...
    for source_name, source_info in sources_info['enhanced_sources'].items():
        ...
        has_creds = "with credentials" if source_info.get('has_credentials', False) else "no credentials"
    ...
    if 'enhanced_adapter' in usage_report:
        ...
        if active_sources:
            ...
            for source_name, stats in active_sources.items():
                ...
                source_display = source_name.replace('_', ' ').title()[:18]
                print(f"{source_display:<20} {calls:<8} {success_rate:<8} {cost:<10} {avg_time:<10}")
        else:
...
def example_configuration():
    ...
    print(f"\nFallback Hierarchy:")
    enabled_sources = [(name, info) for name, info in sources_info['enhanced_sources'].items() 
                      if info['enabled']]
