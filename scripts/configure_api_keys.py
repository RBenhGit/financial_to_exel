...
def test_api_configuration():
    ...
    try:
        ...
        print(
            f"[RESULTS] Results: {test_results['summary']['successful_sources']}/{test_results['summary']['total_sources']} sources working"
        )

        for source_name, result in test_results['sources'].items():
            status = "[OK]" if result.get('success', False) else "[FAIL]"
            print(
                f"  {status} {source_name}: {'Working' if result.get('success', False) else result.get('error', 'Failed')}"
            )
...
def configure_twelve_data():
    """Configure Twelve Data API (paid tier)"""
    print("\n[KEY] Twelve Data Configuration")
    print("=" * 40)
    print("Manage your API key at: https://twelvedata.com/account/api-keys")
    print("Standard paid plan: 500 requests/minute, 10 000 requests/day")

    api_key = input("Enter your Twelve Data API key (or press Enter to skip): ").strip()
...
def main():
    ...
    print("4. Twelve Data (Paid: 500 requests/minute)")
    ...
    while True:
        ...
        if choice == '0':
            ...
            td_config = configure_twelve_data()
            if td_config:
                config['sources']['twelve_data'] = td_config
                save_config(config)
        elif choice == '5':
