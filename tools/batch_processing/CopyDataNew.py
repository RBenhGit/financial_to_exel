...
from core.excel_integration.excel_utils import get_company_name_from_excel, get_period_dates_from_excel, ExcelDataExtractor
...
def categorize_financial_files(files, file_type_label):
    ...
    for file_path in files:
        if "Balance" in file_path:
            ...
            logger.info(
                f"Found {file_type_label} Cash Flow Statement: {os.path.basename(file_path)}"
            )
...
def load_workbooks():
    ...
    for file_type, file_path in files_to_validate:
        try:
            ...
            logger.error(
                f"✗ {file_type} file validation failed", error=e, context={'file_path': file_path}
            )
            raise
    ...
    try:
        ...
        handle_calculation_error(
            "load_workbooks", e, context={'files': [f[1] for f in files_to_validate]}
        )
...
if not Company_Name:
    ...
    logger.warning(
        f"Company name not found using dynamic extraction, using '{get_unknown_company_name()}'"
    )
...
def validated_cell_copy(source_cell, target_cell, context_info):
    ...
    try:
        ...
        if is_valid:
            ...
            logger.debug(
                f"Successfully copied {context_info}: {original_value} -> {validated_value}"
            )
        else:
        ...
        if original_value is None or original_value == "" or original_value == 0:
            validation_warnings.append(
                {"context": context_info, "issue": "Empty or zero value", "value": original_value}
            )
...
def extract_financial_data():
    ...
    financial_metrics = {
        "income": [
            {"name": name, **details}
            for name, details in config.financial_metrics.income_metrics.items()
        ],
        "balance": [
            {"name": name, **details}
            for name, details in config.financial_metrics.balance_metrics.items()
        ],
        "cashflow": [
            {"name": name, **details}
            for name, details in config.financial_metrics.cashflow_metrics.items()
        ],
    }
    ...
    metrics_expected = {
        "income": len(financial_metrics["income"]),
        ...
        for i in row_val:
            if i['income_value'] == metric["name"]:
                ...
                for j in range(9):
                    ...
                    source_cell = Income_wb.cell(
                        row_index, column=excel_config.data_start_column + j
                    )
    ...
    if error_summary['total_errors'] > 0 or error_summary['total_warnings'] > 10:
        logger.save_error_log(
            f"data_extraction_issues_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def generate_data_quality_summary(metrics_found, metrics_expected):
    ...
    for statement_type in metrics_found:
        ...
        logger.info(
            f"{statement_type.upper()} Statement: {found}/{expected} metrics found ({completion_rate:.1f}%)"
        )

        if completion_rate < 100:
            data_quality_report.add_error(
                f"Incomplete {statement_type} data: {found}/{expected} metrics found"
            )
    ...
    if total_errors == 0 and total_warnings == 0:
        ...
        data_quality_report.add_recommendation(
            "Data quality is excellent. Proceed with confidence.", "low"
        )
        ...
        data_quality_report.add_recommendation(
            "Review data quality issues before proceeding with analysis.", "medium"
        )
    else:
        logger.error("\n✗ DATA QUALITY: POOR - Significant issues detected")
        data_quality_report.add_recommendation(
            "Address data quality issues before using for analysis.", "high"
        )
