...
def analyze_msft_dcf_csv():
    ...
    if ev_match:
        ...
        for i, (date, year, projected_fcf, pv_fcf, discount_factor) in enumerate(projections):
            ...
            print(
                f"Year {year}: Projected FCF ${projected_fcf_num/1e6:.0f}M → PV ${pv_fcf_num/1e6:.0f}M"
            )
            total_pv += pv_fcf_num
        ...
        if terminal_match:
            ...
            if ev_unit == "Millions USD":
                ...
                print(
                    f"The actual DCF calculation appears to be {scaling_error:,.0f} times larger than what's exported"
                )
