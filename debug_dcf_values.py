...
def analyze_csv_values():
    ...
    for line in lines:
        if '"Enterprise Value"' in line:
            ...
            scaled, unit, abbrev = get_financial_scale_and_unit(
                expected_raw_value, already_in_millions=False
            )
            print(f"Scaled result: {scaled} {unit}")

            # Test with the incorrect CSV value
            print("\n--- Testing scaling with CSV value (treating as raw) ---")
            scaled2, unit2, abbrev2 = get_financial_scale_and_unit(
                ev_value, already_in_millions=False
            )
