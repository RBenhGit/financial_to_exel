...
class YFinanceLogger:
    ...
    def start_request(self, ticker: str, request_type: str = "market_data") -> str:
        ...
        self.logger.info(f"   Start Time: {self._current_request['start_time'].strftime('%Y-%m-%d %H:%M:%S')}")
        self.logger.info("=" * 80)
    ...
    def log_step(self, step_name: str, details: Dict[str, Any], level: str = "INFO") -> None:
        ...
        step_duration = current_time - (self._step_times[-1] if self._step_times else self._request_start_time)
    ...
    def log_cache_check(self, cache_key: str, cache_hit: bool, cache_age: Optional[float] = None) -> None:
    ...
    def finish_request(self, success: bool, final_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.logger.info("=" * 80)
        self.logger.info(f"{status_emoji} REQUEST COMPLETED | Status: {'SUCCESS' if success else 'FAILED'}")
