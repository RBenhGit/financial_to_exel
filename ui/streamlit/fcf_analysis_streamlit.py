...
from ui.visualization.pb_visualizer import display_pb_analysis
...
# Configure enhanced logging (moved up to define logger before use)
try:
...
from tools.batch_processing.analysis_capture import analysis_capture
...
def initialize_session_state():
    ...
    # Ensure fcf_results synchronization between session_state and financial_calculator
    if hasattr(st.session_state, 'financial_calculator') and st.session_state.financial_calculator:
        if st.session_state.fcf_results and not hasattr(st.session_state.financial_calculator, 'fcf_results'):
            st.session_state.financial_calculator.fcf_results = st.session_state.fcf_results
        elif hasattr(st.session_state.financial_calculator, 'fcf_results') and st.session_state.financial_calculator.fcf_results and not st.session_state.fcf_results:
            st.session_state.fcf_results = st.session_state.financial_calculator.fcf_results
            ...
            if company_path and Path(company_path).exists():
                ...
                if validation['is_valid']:
                    ...
                    with st.spinner("📊 Calculating FCF and fetching market data..."):
                        try:
                            fcf_results = st.session_state.financial_calculator.calculate_all_fcf_types()
                            st.session_state.fcf_results = fcf_results
                            # Ensure financial_calculator also has the results for consistency
                            st.session_state.financial_calculator.fcf_results = fcf_results
                    ...
                    if success and calculator:
                        ...
                        with st.spinner("📊 Calculating FCF using API financial data..."):
                            try:
                                ...
                                st.session_state.fcf_results = fcf_results
                                # Ensure financial_calculator also has the results for consistency
                                calculator.fcf_results = fcf_results
