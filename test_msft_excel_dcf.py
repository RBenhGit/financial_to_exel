...
try:
    ...
    if equity_value:
        ev_scaled, ev_unit, ev_abbrev = get_financial_scale_and_unit(equity_value, already_in_millions=False)
        ...
        try:
            csv_data = create_enhanced_dcf_csv_export(
                dummy_dcf_df, 
                dcf_results, 
                {'discount_rate': 0.1, 'terminal_growth_rate': 0.03, 'projection_years': 10, 'growth_rate_yr1_5': 0.18},
                'MSFT',
                500.0
            )
            ...
            if ev_match:
                ...
                if abs(csv_ev_value - ev_scaled) < 0.001:
                    ...
                    print(f"FAIL: CSV export scaling error: expected {ev_scaled:.3f}, got {csv_ev_value}")
            else:
