...
class TickerValidator:
    ...
    def validate(self, ticker: str) -> ValidationResult:
        ...
        if '.' in clean_ticker:
            ...
            if (
                suffix not in self.EXCHANGE_SUFFIXES
                and self.validation_level == ValidationLevel.STRICT
            ):
                warnings.append(
                    f"Unknown exchange suffix '{suffix}'. Common suffixes: {', '.join(list(self.EXCHANGE_SUFFIXES.keys())[:5])}"
                )
...
class NetworkValidator:
    ...
    def validate_dns_resolution(self, hostname: str = 'finance.yahoo.com') -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=True, metadata={'hostname': hostname, 'test_type': 'dns_resolution'}
            )
...
class DependencyValidator:
    ...
    def validate_package(self, package_name: str, requirements: Dict[str, Any]) -> ValidationResult:
        ...
        if version is None:
            error_level = (
                "CRITICAL_PACKAGE_MISSING"
                if requirements.get('critical', True)
                else "OPTIONAL_PACKAGE_MISSING"
            )
            return ValidationResult(
    ...
    def validate(self) -> ValidationResult:
        ...
        return ValidationResult(
            is_valid=True, warnings=warnings, metadata={'package_results': results}
        )
...
class PreFlightValidator:
    ...
    def validate_ticker(self, ticker: str, use_cache: bool = True) -> ValidationResult:
        ...
        if use_cache and self.cache:
            cached = self.cache.get(
                'ticker', {'ticker': ticker, 'level': self.validation_level.value}
            )
            if cached:
                return cached
        ...
        if use_cache and self.cache:
            self.cache.set(
                'ticker', {'ticker': ticker, 'level': self.validation_level.value}, result
            )
    ...
    def is_ready_for_api_call(
        self, ticker: str, skip_network: bool = False
    ) -> Tuple[bool, List[str]]:
        """
...
def validate_system_ready(skip_network: bool = False) -> Tuple[bool, List[str]]:
    ...
    return validator.is_ready_for_api_call(
        "AAPL", skip_network
...
if __name__ == "__main__":
    ...
    for category, result in results.items():
        print(
            f"{category:12} -> {'✓' if result.is_valid else '✗'} {result.error_message or 'Valid'}"
        )
