...
class DataSourceManager:
    ...
    def __init__(self, base_path: str = "."):
        ...
        self.adapter = UnifiedDataAdapter(
            config_file=str(self.base_path / "data_sources_config.json"),
    ...
    def interactive_setup(self):
        ...
        sources_to_configure = [
            (DataSourceType.ALPHA_VANTAGE, "Alpha Vantage", "Free tier: 5 calls/min, 500 calls/month"),
            (DataSourceType.FINANCIAL_MODELING_PREP, "Financial Modeling Prep", "Free tier: 250 calls/month"),
                ...
                if api_key:
                    ...
                    if success:
                        ...
                        if source_type == DataSourceType.POLYGON:
                            cost_per_call = input("Cost per API call (USD, default 0.003): ").strip()
                            if cost_per_call:
                                try:
                                    cost = float(cost_per_call)
                                    self.adapter.configure_source(source_type, api_key, cost_per_call=cost)
                                except ValueError:
        ...
        for source_type, config in self.adapter.configurations.items():
            if config.is_enabled:
                if source_type == DataSourceType.EXCEL:
                    ...
                    active_sources.append(f"✓ {source_type.value.replace('_', ' ').title()} (Priority: {config.priority.value})")
    ...
    def test_sources(self, ticker: str = "AAPL") -> Dict[str, Any]:
        ...
        try:
            ...
            if response.success:
                print(f"  ✅ Success from {response.source_type.value if response.source_type else 'unknown'}!")
                print(f"  📊 Quality score: {response.quality_metrics.overall_score if response.quality_metrics else 'N/A'}")
        ...
        for source_name, stats in report['sources'].items():
            if stats['total_calls'] > 0:
                ...
                if stats['monthly_limit']:
                    print(f"  Monthly Usage: {stats['monthly_calls']}/{stats['monthly_limit']} ({stats['monthly_calls']/stats['monthly_limit']:.1%})")
                if stats['last_used']:
        ...
        try:
            ...
            for source_type, config in self.adapter.configurations.items():
                config_info = {
                    ...
                    'has_credentials': config.credentials is not None and bool(config.credentials.api_key)
                }
