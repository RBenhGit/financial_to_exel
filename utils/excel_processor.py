...
class UnifiedExcelProcessor:
    ...
    def load_excel_to_dataframe(self, file_path: str, use_cache: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Loaded Excel file: {file_path} ({len(df)} rows, {len(df.columns)} columns)"
            )
            return df
    ...
    def find_company_name(
        self, df: pd.DataFrame, search_positions: Optional[List[Tuple[int, int]]] = None
    ) -> Optional[str]:
        """
    ...
    def load_company_financial_statements(
        self, company_folder: str
