...
class FieldNormalizer:
    ...
    def extract_field_value(
        self, data: Any, standard_field: str, api_source: str, context: str = "unknown"
    ) -> Optional[float]:
        """
        ...
        logger.debug(
            f"Attempting to extract {standard_field} from {api_source} using variants: {field_variants}"
        )
            ...
            try:
                ...
                if value is not None:
                    ...
                    if normalized_value is not None:
                        logger.debug(
                            f"Successfully extracted {standard_field}={normalized_value} using field '{field_name}' from {api_source}"
                        )
                        return normalized_value
                    else:
                        logger.debug(
                            f"Field '{field_name}' found but value could not be normalized: {value}"
                        )
        ...
        logger.warning(
            f"Could not extract {standard_field} from {api_source} data (context: {context})"
        )
        return None
    ...
    def _extract_from_dict(self, data: dict, field_name: str, api_source: str) -> Any:
        ...
        if api_source == "alpha_vantage":
            for report_type in ["annualReports", "quarterlyReports"]:
                if (
                    report_type in data
                    and isinstance(data[report_type], list)
                    and len(data[report_type]) > 0
                ):
                    nested_value = self._extract_from_dict(
                        data[report_type][0], field_name, api_source
                    )
    ...
    def calculate_free_cash_flow(
        ...
        if operating_cash_flow is None or capital_expenditures is None:
            logger.warning(
                f"Cannot calculate FCF: OCF={operating_cash_flow}, CapEx={capital_expenditures}"
            )
            return None
        ...
        if capex_handling == 'negative_reported':
            ...
            if capital_expenditures > 0:
                logger.debug(
                    f"CapEx expected to be negative for {api_source}, but got positive: {capital_expenditures}"
                )
        ...
        logger.debug(
            f"FCF calculation for {api_source}: OCF({operating_cash_flow}) - CapEx({capital_expenditures}) = {free_cash_flow}"
        )

        return free_cash_flow

    def extract_financial_metrics(
        self, data: Dict[str, Any], api_source: str, ticker: str = "unknown"
        ...
        if 'operating_cash_flow' in metrics and 'capital_expenditures' in metrics:
            metrics['free_cash_flow'] = self.calculate_free_cash_flow(
                metrics['operating_cash_flow'], metrics['capital_expenditures'], api_source
            )
