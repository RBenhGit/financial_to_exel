...
class YfinanceConverter:
        ...
        for yf_field, standard_field in cls.FIELD_MAPPINGS.items():
            if yf_field in df.index:
                ...
                if normalized_value is not None:
                    ...
                    logger.debug(
                        f"yfinance DataFrame: Mapped {yf_field} -> {standard_field}: {normalized_value}"
                    )
        ...
        for yf_field, standard_field in cls.FIELD_MAPPINGS.items():
            if yf_field in data_dict:
                ...
                if normalized_value is not None:
                    standardized[standard_field] = normalized_value
                    logger.debug(
                        f"yfinance dict: Mapped {yf_field} -> {standard_field}: {normalized_value}"
                    )
    ...
    @classmethod
    def handle_quarterly_vs_annual(
        cls, annual_data: pd.DataFrame, quarterly_data: pd.DataFrame, prefer_quarterly: bool = False
    ) -> Dict[str, Any]:
        """
