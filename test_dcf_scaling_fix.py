...
def test_get_financial_scale_and_unit():
    ...
    for i, (input_val, already_millions, expected_scaled, expected_unit) in enumerate(test_cases):
        try:
            ...
            scaled_ok = abs(scaled_val - expected_scaled) < 0.01  # Allow small floating point differences
            unit_ok = unit == expected_unit
            
            if scaled_ok and unit_ok:
                print(f"  PASS Test {i+1}: {input_val} ({'millions' if already_millions else 'raw'}) -> {scaled_val:.3f} {unit}")
                passed += 1
            else:
                print(f"  FAIL Test {i+1}: {input_val} ({'millions' if already_millions else 'raw'}) -> {scaled_val:.3f} {unit}")
...
def test_microsoft_dcf_scaling():
    ...
    ev_scaled, ev_unit, ev_abbrev = get_financial_scale_and_unit(microsoft_dcf_results['enterprise_value'])
    print(f"  Enterprise Value: {ev_scaled:.3f} {ev_unit}")
    
    # Test equity value scaling  
    eq_scaled, eq_unit, eq_abbrev = get_financial_scale_and_unit(microsoft_dcf_results['equity_value'])
    print(f"  Equity Value: {eq_scaled:.3f} {eq_unit}")
    
    # Test terminal value scaling
    tv_scaled, tv_unit, tv_abbrev = get_financial_scale_and_unit(microsoft_dcf_results['terminal_value'])
