...
class FCFCalculator:
    ...
    def calculate_fcf_growth_rates(
        self, fcf_results: Dict[str, List[float]], periods: List[int] = None
    ...
    def _calculate_growth_rates_for_values(
        self, values: List[float], periods: List[int]
    ) -> Dict[str, float]:
        """
    ...
    def _calculate_average_growth_rates(
        self, growth_rates: Dict[str, Dict[str, float]], periods: List[int]
    ) -> Dict[str, float]:
        """
    ...
    def calculate_fcf_metrics_summary(self, fcf_results: Dict[str, List[float]]) -> Dict[str, Any]:
        ...
        """
        summary = {'latest_values': {}, 'growth_rates': {}, 'statistics': {}, 'average_fcf': {}}
    ...
    def format_fcf_data_for_display(
        self, fcf_results: Dict[str, List[float]], years: List[int] = None
    ) -> pd.DataFrame:
        """
            ...
            if avg_fcf > 0:
                ...
                if variability < 0.2:  # Low variability
                    ...
                    recommendation['warnings'].append(
                        "High variability between FCF calculation methods"
                    )

        # Generate final recommendation
        if (
            recommendation['overall_trend'] == 'positive'
            and recommendation['consistency'] == 'high'
        ):
            recommendation['recommendation'] = 'buy'
        elif (
            recommendation['overall_trend'] == 'negative' or recommendation['consistency'] == 'low'
        ):
...
def calculate_fcf_growth_rates(
    fcf_results: Dict[str, List[float]], periods: List[int] = None
) -> Dict[str, Dict[str, float]]:
    """
...
def format_fcf_data_for_display(
    fcf_results: Dict[str, List[float]], years: List[int] = None
) -> pd.DataFrame:
    """
