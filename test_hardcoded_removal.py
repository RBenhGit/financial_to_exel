...
from config import (
    get_default_company_name, get_unknown_company_name, get_unknown_ticker,
    get_unknown_fcf_type, get_test_company_name, get_test_company_ticker
)
...
def test_data_processor_no_hardcoded():
    ...
    mock_fcf_results = {
        'fcf_data': {'years': [2020, 2021], 'FCFF': [100, 120], 'FCFE': [80, 100]}
    }
    ...
    try:
        ...
        assert configured_name in fig1.layout.title.text, f"Plot should use configured name '{configured_name}'"
...
def test_financial_calculator_no_hardcoded():
    ...
    try:
        ...
        assert calc.company_name == configured_unknown, f"Should use configured unknown name '{configured_unknown}', got '{calc.company_name}'"
        ...
        if os.path.exists(filename):
            with open(filename, 'r', encoding='utf-8') as f:
                ...
                for pattern in hardcoded_patterns:
                    if pattern in content and 'get_' not in content[max(0, content.find(pattern) - 50):content.find(pattern)]:
