...
def test_centralized_system():
    ...
    try:
        ...
        print(f"\nData Processor Stats:")
        print(f"  - Supported metrics: {len(processing_stats['processing_stats']['supported_metrics'])}")
        print(f"  - FCF types: {processing_stats['processing_stats']['fcf_types']}")
        print(f"  - Growth rate periods: {processing_stats['processing_stats']['growth_rate_periods']}")
