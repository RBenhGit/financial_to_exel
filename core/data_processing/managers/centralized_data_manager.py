...
class CentralizedDataManager:
    ...
    def fetch_market_data(
        ...
        try:
            ...
            # Check if Yahoo Finance is available via circuit breaker
            if not self.rate_limiter.can_make_request('yahoo_finance'):
                logger.warning(f"Yahoo Finance unavailable for {ticker}, trying fallback sources")
                return self._fetch_fallback_market_data(ticker)
            ...
            self.yf_logger.log_step(
                "Enhanced Rate Limiting Configuration",
                {
                    ...
                    "circuit_breaker_enabled": self.rate_limiter.config.circuit_breaker_enabled,
                    "adaptive_rate_limiting": self.rate_limiter.config.adaptive_rate_limiting,
                    "request_queue_enabled": self.rate_limiter.config.request_queue_enabled,
                },
                level="DEBUG",
            )
            ...
            for attempt in range(max_retries):
                try:
                    # Use enhanced rate limiter for request management
                    with self.rate_limiter.rate_limited_request('yahoo_finance', attempt):
                    ...
                    if market_data:
                        ...
                        logger.info(
                            f"Successfully fetched market data for {ticker} on attempt {attempt + 1}"
                        )
                        ...
                        logger.warning(
                            f"No market data extracted for {ticker} on attempt {attempt + 1}"
                        )
                        self.yf_logger.log_step(
                    ...
                    if hasattr(e, "response") and e.response is not None:
                        ...
                        if status_code == 429:
                            logger.warning(
                                f"HTTP 429 (Rate Limited) for {ticker} on attempt {attempt + 1}"
                            )
                            self.yf_logger.log_error(
                                e, {**error_context, "error_category": "rate_limiting"}
                            )
                            if attempt < max_retries - 1:
                                continue
                        elif status_code in [500, 502, 503, 504]:
                            logger.warning(
                                f"HTTP {status_code} (Server Error) for {ticker} on attempt {attempt + 1}"
                            )
                            self.yf_logger.log_error(
                                e, {**error_context, "error_category": "server_error"}
                            )
                            if attempt < max_retries - 1:
                                continue
                        else:
                        ...
                        self.yf_logger.log_error(
                            e, {**error_context, "error_category": "request_error"}
                        )
                    ...
                    is_retryable = (
                        ...
                        or "expecting value" in error_str  # JSON parsing errors from rate limiting
                    ...
                    if is_retryable and attempt < max_retries - 1:
                        ...
                        self.yf_logger.log_error(
                            e, {**error_context, "action": "max_retries_exhausted"}
                        )
            ...
            logger.warning(
                f"Primary data source (yfinance) failed for {ticker}, trying fallback sources..."
            )
            self.yf_logger.log_step(
    ...
    def _extract_market_data(self, info: Dict[str, Any], stock) -> Optional[Dict[str, Any]]:
        ...
        try:
                ...
                self.yf_logger.log_data_extraction(
                    {
                        ...
                        "price_values_found": {
                            source: info.get(source) for source in price_sources
                        },
                    }
                )
                            ...
                            self.yf_logger.log_data_extraction(
                                {
                                    ...
                                    "close_column_exists": (
                                        "Close" in hist.columns if not hist.empty else False
                                    ),
                                }
                            )
            ...
            shares_sources = [
                ...
                "basicAvgSharesOutstanding",
                "weightedAvgSharesOutstanding",
                ...
                self.yf_logger.log_data_extraction(
                    {
                        ...
                        "shares_values_found": {
                            source: info.get(source) for source in shares_sources
                        },
                        "market_cap_values_found": {
                            source: info.get(source) for source in market_cap_sources
                        },
                    }
                )
            ...
            company_name = (info.get("longName") or info.get("shortName") or ticker_symbol).strip()
            ...
            result = {
                ...
                "market_cap": (
                    validated_market_cap / self.data_config["value_scale"]
                    if validated_market_cap
                    else 0
                ),
