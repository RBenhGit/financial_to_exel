...
class DataQualityReport:
    ...
    def calculate_scores(self):
        ...
        total_metrics = (
            sum(len(data) for data in self.missing_data.values()) if self.missing_data else 0
        )
        missing_metrics = (
            sum(data.count(True) for data in self.missing_data.values()) if self.missing_data else 0
        )
...
class FinancialDataValidator:
    ...
    def validate_cell_value(
        self, value: Any, expected_type: type = float, allow_zero: bool = True, context: str = ""
    ) -> Tuple[Any, bool]:
        """
        ...
        try:
            ...
            if expected_type in (int, float):
                ...
                if (
                    converted_value < rules["min_reasonable_value"]
                    or converted_value > rules["max_reasonable_value"]
                ):
                    self.report.add_warning(
                        f"Value outside reasonable bounds: {converted_value:,.0f}", context
                    )
    ...
    def validate_data_series(
        self, values: List[Any], metric_name: str, min_years: int = None
        ...
        min_required = (
            min_years or self.validation_rules["numeric_thresholds"]["min_years_required"]
        )
                    ...
                    if metric.lower() in str(row.iloc[2]).lower():
                        ...
                        for val in row.iloc[3:]:
                            if pd.notna(val) and val != '':
                                cleaned_val, _ = self.validate_cell_value(
                                    val, float, True, f"{statement_type}.{metric}"
                                )
                                values.append(cleaned_val)

                        if len(values) == 0:
                            self.report.add_error(
                                f"No valid data found for {metric} in {statement_type}"
                            )
                        else:
    ...
    def validate_metric_extraction(
        self, df: pd.DataFrame, metric_name: str, extracted_values: List[float]
    ) -> bool:
        """
        ...
        if len(extracted_values) < min_years:
            self.report.add_warning(
                ...
                f"minimum {min_years} recommended",
                context,
            )
    ...
    def validate_fcf_date_correlation(
        self, 
        source_report_dates: List[str], 
        fcf_calculation_dates: List[str], 
        fcf_values: List[float],
        calculation_context: str = "FCF Analysis"
    ) -> bool:
        """
        Validate that FCF calculations correlate to appropriate source report dates
        
        Args:
            source_report_dates: List of dates from financial statements
            fcf_calculation_dates: List of dates used in FCF calculations
            fcf_values: FCF values for validation
            calculation_context: Context description for reporting
            
        Returns:
            bool: Whether date correlation is valid
        """
        context = f"Date correlation validation for {calculation_context}"
        is_valid = True
        
        # Check if we have dates to validate
        if not source_report_dates and not fcf_calculation_dates:
            self.report.add_warning("No dates available for correlation validation", context)
            return True
        ...
        try:
            ...
            for date_str in source_report_dates:
                if date_str and date_str != "Unknown":
                    try:
                        parsed_date = datetime.strptime(date_str.split()[0], "%Y-%m-%d")
                        source_dates_parsed.append(parsed_date)
                    except (ValueError, IndexError):
                        try:
                            parsed_date = datetime.strptime(str(date_str)[:4], "%Y")
                            source_dates_parsed.append(parsed_date)
                        except ValueError:
                            continue
                            
            for date_str in fcf_calculation_dates:
                if date_str and date_str != "Unknown":
                    try:
                        parsed_date = datetime.strptime(date_str.split()[0], "%Y-%m-%d")
                        fcf_dates_parsed.append(parsed_date)
                    except (ValueError, IndexError):
                        try:
                            parsed_date = datetime.strptime(str(date_str)[:4], "%Y")
                            fcf_dates_parsed.append(parsed_date)
            ...
            if source_dates_parsed and fcf_dates_parsed:
                latest_source = max(source_dates_parsed)
                latest_fcf = max(fcf_dates_parsed)
                
                # Allow up to 1 year difference for reporting lag
                date_diff_years = abs((latest_source - latest_fcf).days) / 365.25
                
                if date_diff_years > 1.5:
                    self.report.add_warning(
                        f"Significant date mismatch: Latest source date {latest_source.strftime('%Y-%m-%d')}, "
                        f"latest FCF calculation date {latest_fcf.strftime('%Y-%m-%d')} "
                        f"({date_diff_years:.1f} years apart)",
                        context
                    )
                    is_valid = False
                ...
                source_age_years = (current_date - latest_source).days / 365.25
                
                if source_age_years > 2.0:
                    self.report.add_warning(
                        f"Source data may be outdated: Latest report date {latest_source.strftime('%Y-%m-%d')} "
                        f"is {source_age_years:.1f} years old",
                        context
                    )
                    
            # Validate FCF values have corresponding dates
            if len(fcf_values) != len(fcf_calculation_dates):
                self.report.add_warning(
                    f"FCF values count ({len(fcf_values)}) doesn't match dates count ({len(fcf_calculation_dates)})",
                    context
                )
                is_valid = False
                
        except Exception as e:
            self.report.add_error(f"Error during date correlation validation: {str(e)}", context)
            is_valid = False
            
        return is_valid
        
    def validate_report_date_consistency(self, financial_data: Dict) -> bool:
        """
        Validate consistency of report dates across different financial statements
        
        Args:
            financial_data: Dictionary containing financial statement data
            
        Returns:
            bool: Whether dates are consistent across statements
        """
        context = "Report date consistency validation"
        is_valid = True
        statement_dates = {}
        
        try:
            # Extract dates from each statement
            for statement_name, data in financial_data.items():
                if isinstance(data, pd.DataFrame) and not data.empty:
                    # Try to extract dates from DataFrame index or columns
                    dates = []
                    
                    # Check if index contains dates
                    if hasattr(data.index, 'dtype') and 'datetime' in str(data.index.dtype):
                        dates = [date.strftime('%Y-%m-%d') for date in data.index]
                    
                    # Check for date columns
                    date_columns = [col for col in data.columns if 'date' in str(col).lower()]
                    if date_columns and not dates:
                        for date_col in date_columns:
                            try:
                                col_dates = pd.to_datetime(data[date_col], errors='coerce')
                                valid_dates = col_dates.dropna()
                                if len(valid_dates) > 0:
                                    dates = [date.strftime('%Y-%m-%d') for date in valid_dates]
                                    break
                            except:
                                continue
                    
                    if dates:
                        statement_dates[statement_name] = dates
            
            # Compare dates across statements
            if len(statement_dates) > 1:
                reference_dates = None
                reference_statement = None
                
                for statement_name, dates in statement_dates.items():
                    if reference_dates is None:
                        reference_dates = set(dates)
                        reference_statement = statement_name
                    else:
                        ...
                        if len(overlap) == 0:
                            self.report.add_warning(
                                f"No common dates between {reference_statement} and {statement_name}",
                                context
                            )
                            is_valid = False
                        elif len(overlap) / len(reference_dates) < 0.5:
                            self.report.add_warning(
                                f"Limited date overlap between {reference_statement} and {statement_name}: "
                                f"{len(overlap)} common dates",
                                context
                            )
                            
        except Exception as e:
            self.report.add_error(f"Error during report date consistency validation: {str(e)}", context)
            is_valid = False
