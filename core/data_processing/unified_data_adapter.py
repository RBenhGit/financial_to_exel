...
from ..data_sources.interfaces.data_sources import (
    DataSourceType,
    DataSourcePriority,
    DataSourceConfig,
    ApiCredentials,
...
class UsageStatistics:
    ...
    def update_stats(self, response: DataSourceResponse) -> None:
...
class UnifiedDataAdapter:
    ...
    def _can_use_provider(
        self, source_type: DataSourceType, stats: Optional[UsageStatistics]
    ) -> bool:
    ...
    def fetch_data(self, request: FinancialDataRequest) -> DataSourceResponse:
        ...
        with self._lock:
            ...
            return DataSourceResponse(
                success=False, error_message="All configured data sources failed"
            )
    ...
    def get_usage_report(self) -> Dict[str, Any]:
        ...
        for source_type, stats in self.usage_stats.items():
            ...
            report['sources'][source_type.value] = {
                ...
                'monthly_cost': stats.monthly_cost,
                'monthly_limit': (
                    config.credentials.monthly_limit if config and config.credentials else None
                ),
                'average_response_time': stats.average_response_time,
                'last_used': stats.last_used.isoformat() if stats.last_used else None,
            }
