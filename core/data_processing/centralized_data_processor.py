...
class CentralizedDataProcessor:
    ...
    def extract_financial_metrics(
        ...
        cache_key = self.data_manager._generate_cache_key(
            'financial_metrics', {'company': company_folder}
        )
        ...
        try:
            ...
            missing_datasets = [
                ...
                if ds not in financial_data or financial_data[ds].empty
            ]
    ...
    def _extract_all_metrics(
        self, financial_data: Dict[str, pd.DataFrame], errors: List[str], warnings: List[str]
    ) -> Dict[str, List[float]]:
        ...
        metrics['ebit'] = self._extract_metric_with_ltm(
            income_fy, income_ltm, "EBIT", errors, warnings
        )
        metrics['net_income'] = self._extract_metric_with_ltm(
            income_fy, income_ltm, "Net Income", errors, warnings
        )
        metrics['tax_expense'] = self._extract_metric_with_ltm(
            income_fy, income_ltm, "Income Tax Expense", errors, warnings
        )
        metrics['ebt'] = self._extract_metric_with_ltm(
            income_fy, income_ltm, "EBT", errors, warnings
        )

        # Balance sheet metrics
        metrics['current_assets'] = self._extract_metric_with_ltm(
            balance_fy, balance_ltm, "Total Current Assets", errors, warnings
        )
        metrics['current_liabilities'] = self._extract_metric_with_ltm(
            balance_fy, balance_ltm, "Total Current Liabilities", errors, warnings
        )

        # Cash flow metrics
        metrics['depreciation_amortization'] = self._extract_metric_with_ltm(
            cashflow_fy, cashflow_ltm, "Depreciation & Amortization", errors, warnings
        )
        metrics['operating_cash_flow'] = self._extract_metric_with_ltm(
            cashflow_fy, cashflow_ltm, "Cash from Operations", errors, warnings
        )
        metrics['capex'] = self._extract_metric_with_ltm(
            cashflow_fy, cashflow_ltm, "Capital Expenditure", errors, warnings
        )
        metrics['debt_issued'] = self._extract_metric_with_ltm(
            cashflow_fy, cashflow_ltm, "Long-Term Debt Issued", errors, warnings
        )
        metrics['debt_repaid'] = self._extract_metric_with_ltm(
            cashflow_fy, cashflow_ltm, "Long-Term Debt Repaid", errors, warnings
        )

        # Calculate derived metrics
        metrics['net_borrowing'] = self._calculate_net_borrowing(
            metrics['debt_issued'], metrics['debt_repaid']
        )
    ...
    def _calculate_net_borrowing(
        self, debt_issued: List[float], debt_repaid: List[float]
    ) -> List[float]:
    ...
    def _calculate_working_capital_changes(
        self, current_assets: List[float], current_liabilities: List[float]
    ) -> List[float]:
    ...
    def calculate_fcf(self, company_folder: str, force_reload: bool = False) -> ProcessingResult:
        ...
        # Generate cache key
        cache_key = self.data_manager._generate_cache_key(
            'fcf_results', {'company': company_folder}
        )
        ...
        try:
            ...
            # Calculate FCF types using centralized FinancialCalculator
            fcff = self._calculate_fcff(metrics, company_folder)
            fcfe = self._calculate_fcfe(metrics, company_folder)
            lfcf = self._calculate_lfcf(metrics, company_folder)
    ...
    def _calculate_fcff(self, metrics: FinancialMetrics, company_folder: str) -> List[float]:
        """
        Calculate Free Cash Flow to Firm using centralized FinancialCalculator

        Args:
            metrics: Financial metrics (kept for backward compatibility)
            company_folder: Company folder path for FinancialCalculator

        Returns:
            List[float]: FCFF values
        """
        try:
            # Use centralized FinancialCalculator for consistent calculation
            financial_calculator = FinancialCalculator(company_folder)
            fcff_result = financial_calculator.calculate_fcf_to_firm(use_var_input_data=False)

            if not fcff_result:
                logger.warning(f"FinancialCalculator returned no FCFF data for {company_folder}")
                return []

            logger.info(f"Successfully calculated FCFF using centralized method for {company_folder}")
            return fcff_result

        except Exception as e:
            logger.error(f"Error calculating FCFF with FinancialCalculator for {company_folder}: {e}")
            # Fallback to ensure functionality doesn't break
            if not metrics.ebit:
                return []
            ...
            for i in range(len(metrics.ebit)):
                fcff_value = (
                    metrics.ebit[i] * (1 - (metrics.tax_rates[i] if i < len(metrics.tax_rates) else 0.25))
                    + (metrics.depreciation_amortization[i] if i < len(metrics.depreciation_amortization) else 0)
                    - (metrics.working_capital_changes[i] if i < len(metrics.working_capital_changes) else 0)
                    - abs(metrics.capex[i] if i < len(metrics.capex) else 0)
                )
                fcff.append(fcff_value)
            return fcff

    def _calculate_fcfe(self, metrics: FinancialMetrics, company_folder: str) -> List[float]:
        """
        Calculate Free Cash Flow to Equity using centralized FinancialCalculator
        ...
        Returns:
            List[float]: FCFE values
        """
        try:
            # Use centralized FinancialCalculator for consistent calculation
            financial_calculator = FinancialCalculator(company_folder)
            fcfe_result = financial_calculator.calculate_fcf_to_equity(use_var_input_data=False)

            if not fcfe_result:
                logger.warning(f"FinancialCalculator returned no FCFE data for {company_folder}")
                return []

            logger.info(f"Successfully calculated FCFE using centralized method for {company_folder}")
            return fcfe_result

        except Exception as e:
            logger.error(f"Error calculating FCFE with FinancialCalculator for {company_folder}: {e}")
            # Fallback to ensure functionality doesn't break
            ...
            for i in range(len(metrics.net_income)):
                fcfe_value = (
                    metrics.net_income[i]
                    + (metrics.depreciation_amortization[i] if i < len(metrics.depreciation_amortization) else 0)
                    - (metrics.working_capital_changes[i] if i < len(metrics.working_capital_changes) else 0)
                    - abs(metrics.capex[i] if i < len(metrics.capex) else 0)
                    + (metrics.net_borrowing[i] if i < len(metrics.net_borrowing) else 0)
                )
                fcfe.append(fcfe_value)
            return fcfe

    def _calculate_lfcf(self, metrics: FinancialMetrics, company_folder: str) -> List[float]:
        """
        Calculate Levered Free Cash Flow using centralized FinancialCalculator
        ...
        Returns:
            List[float]: Levered FCF values
        """
        try:
            # Use centralized FinancialCalculator for consistent calculation
            financial_calculator = FinancialCalculator(company_folder)
            lfcf_result = financial_calculator.calculate_levered_fcf(use_var_input_data=False)

            if not lfcf_result:
                logger.warning(f"FinancialCalculator returned no LFCF data for {company_folder}")
                return []

            logger.info(f"Successfully calculated LFCF using centralized method for {company_folder}")
            return lfcf_result

        except Exception as e:
            logger.error(f"Error calculating LFCF with FinancialCalculator for {company_folder}: {e}")
            # Fallback to ensure functionality doesn't break
            ...
            for i in range(len(metrics.operating_cash_flow)):
                lfcf_value = metrics.operating_cash_flow[i] - abs(metrics.capex[i] if i < len(metrics.capex) else 0)
    ...
    def get_processing_statistics(self) -> Dict[str, Any]:
        ...
        return {'cache_stats': cache_stats, 'processing_stats': processing_stats}
