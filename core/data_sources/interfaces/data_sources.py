...
class FinancialDataRequest:
    ...
    def __post_init__(self):
        ...
        if self.pb_analysis_mode:
            ...
            pb_data_types = ['historical_prices', 'quarterly_balance_sheet', 'historical_fundamentals']
...
class FinancialDataProvider(ABC):
    ...
    def _enforce_rate_limit(self) -> None:
        ...
        min_interval = (
            self.config.credentials.rate_limit_period / self.config.credentials.rate_limit_calls
        )
...
class AlphaVantageProvider(FinancialDataProvider):
    ...
    def _fetch_fundamental_data(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            overview_response = self._session.get(
                overview_url, timeout=self.config.credentials.timeout
            )
                ...
                result.update(
                    {
                        ...
                        "pe_ratio": (
                            ...
                            if overview_data.get("PERatio") != "None"
                            else None
                        ),
                        "pb_ratio": (
                            float(overview_data.get("PriceToBookRatio", 0))
                            if overview_data.get("PriceToBookRatio") != "None"
                            else None
                        ),
                        "dividend_yield": (
                            float(overview_data.get("DividendYield", 0))
                            if overview_data.get("DividendYield") != "None"
                            else None
                        ),
                        "eps": (
                            float(overview_data.get("EPS", 0))
                            if overview_data.get("EPS") != "None"
                            else None
                        ),
                        "revenue_ttm": float(overview_data.get("RevenueTTM", 0)),
                        "profit_margin": (
                            float(overview_data.get("ProfitMargin", 0))
                            if overview_data.get("ProfitMargin") != "None"
                            else None
                        ),
                        "beta": (
                            float(overview_data.get("Beta", 0))
                            if overview_data.get("Beta") != "None"
                            else None
                        ),
    ...
    def _fetch_financial_statements(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            balance_response = self._session.get(
                balance_url, timeout=self.config.credentials.timeout
            )
            ...
            cashflow_response = self._session.get(
                cashflow_url, timeout=self.config.credentials.timeout
            )
    ...
    def _calculate_fcf_from_alpha_vantage(
        self, cashflow_reports: List[Dict[str, Any]]
        ...
        try:
            ...
            standardized_data = AlphaVantageConverter.convert_financial_data(
                {"annualReports": cashflow_reports}
            )
            ...
            if fcf_result.get("success"):
                ...
                logger.error(
                    f"Unified FCF calculation failed for Alpha Vantage: {fcf_result.get('error')}"
                )
                return None
...
class FinancialModelingPrepProvider(FinancialDataProvider):
    ...
    def _fetch_fundamental_data(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            profile_url = (
                f"{self.base_url}/profile/{ticker}?apikey={self.config.credentials.api_key}"
            )
            profile_response = self._session.get(
                profile_url, timeout=self.config.credentials.timeout
            )
                ...
                result.update(
                    {
                        ...
                        "dividend_yield": (
                            float(profile.get("lastDiv", 0)) if profile.get("lastDiv") else None
                        ),
...
class PolygonProvider(FinancialDataProvider):
    ...
    def validate_credentials(self) -> bool:
        ...
        try:
            test_url = (
                f"{self.base_url}/v2/last/trade/AAPL?apikey={self.config.credentials.api_key}"
            )
            response = self._session.get(test_url, timeout=10)
    ...
    def _fetch_fundamental_data(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            details_response = self._session.get(
                details_url, timeout=self.config.credentials.timeout
            )
    ...
    def _fetch_financial_statements_polygon(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            financials_response = self._session.get(
                financials_url, timeout=self.config.credentials.timeout
            )
    ...
    def _calculate_fcf_from_polygon(
        self, financial_results: List[Dict[str, Any]]
    ) -> Optional[Dict[str, Any]]:
...
class ExcelDataProvider(FinancialDataProvider):
    ...
    def _standardize_excel_data(
        self, excel_data: Dict[str, pd.DataFrame], ticker: str
    ) -> Dict[str, Any]:
        """Convert Excel data to standardized format"""
        try:
            # Extract key financial metrics from Excel data
            result = {"ticker": ticker.upper(), "source": "excel_files", "financial_statements": {}}
            ...
            if not hist.empty and info:
                ...
                return {
                    "current_price": (
                        float(latest['Close'])
                        if 'Close' in latest
                        else float(info.get('currentPrice', 0))
                    ),
                    "volume": (
                        int(latest['Volume']) if 'Volume' in latest else int(info.get('volume', 0))
                    ),
        ...
        try:
            ...
            if info:
                result = {
                    "market_cap": float(info.get('marketCap', 0)),
                    "pe_ratio": (
                        float(info.get('trailingPE', 0)) if info.get('trailingPE') else None
                    ),
                    "pb_ratio": (
                        float(info.get('priceToBook', 0)) if info.get('priceToBook') else None
                    ),
                    "dividend_yield": (
                        float(info.get('dividendYield', 0)) if info.get('dividendYield') else None
                    ),
                    "eps": float(info.get('trailingEps', 0)) if info.get('trailingEps') else None,
                    "revenue_ttm": float(info.get('totalRevenue', 0)),
                    "profit_margin": (
                        float(info.get('profitMargins', 0)) if info.get('profitMargins') else None
                    ),
