...
class EnhancedLogger:
    ...
    def _log_with_context(
        self, level: int, message: str, context: Dict = None, error: Exception = None, **kwargs
    ):
...
def with_error_handling(
    error_type: type = FinancialAnalysisError,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if log_errors:
                    logger.error(
                        f"Error in {func.__name__}: {str(e)}", context=error_context, error=e
                    )
                ...
                if re_raise:
                    if isinstance(e, FinancialAnalysisError):
                        ...
                        raise error_type(
                            f"Error in {func.__name__}: {str(e)}", context=error_context
                        ) from e
...
def validate_excel_file(file_path: str) -> bool:
    ...
    if not file_path.lower().endswith(('.xlsx', '.xls')):
        raise ExcelDataError(
            f"File is not an Excel file: {file_path}",
...
def validate_financial_data(data: Any, data_type: str, required_fields: List[str] = None) -> bool:
    ...
    if data is None:
        raise ValidationError(
            f"{data_type} data is None", error_code="NULL_DATA", context={'data_type': data_type}
        )
...
def handle_calculation_error(func_name: str, error: Exception, context: Dict = None) -> None:
    ...
    logger.save_error_log(
        f"calculation_error_{func_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def create_error_summary(errors: List[Dict], warnings: List[Dict]) -> Dict:
    ...
    if summary['total_errors'] > 0:
        summary['recommendations'].append(
            "Review error logs and fix critical issues before proceeding"
        )

    if summary['total_warnings'] > 10:
        summary['recommendations'].append(
            "High number of warnings detected - consider data quality review"
        )

    if 'ExcelDataError' in summary['error_types']:
        summary['recommendations'].append(
            "Excel file format issues detected - verify file structure"
        )
